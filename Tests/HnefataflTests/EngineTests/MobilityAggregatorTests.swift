...
@Suite("MobilityAggregator Tests")
struct MobilityAggregatorTests {

    @Test("Returns value on Copenhagen start for attacker")
    func copenhagenAttacker() {
        let pos = Position.copenhagenStart()
        let score = MobilityAggregator.compute(position: pos, player: .attacker)
    ...
    func copenhagenDefender() {
        let pos = Position.copenhagenStart()
        let score = MobilityAggregator.compute(position: pos, player: .defender)
        #expect(score > Int.min)
        #expect(score < Int.max)
    }
    ...
    func oppositeSignsOnStart() {
        let pos = Position.copenhagenStart()
        let atkScore = MobilityAggregator.compute(position: pos, player: .attacker)
        let defScore = MobilityAggregator.compute(position: pos, player: .defender)
